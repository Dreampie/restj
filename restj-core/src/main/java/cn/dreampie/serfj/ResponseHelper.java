...
import cn.dreampie.serfj.annotations.DoNotRenderPage;
import cn.dreampie.serfj.config.ConfigFileIOException;
import cn.dreampie.log.Logger;
import cn.dreampie.serfj.serializers.FileSerializer;
import cn.dreampie.serfj.serializers.ObjectSerializer;
...
public class ResponseHelper {
    ...
    private static final Logger LOGGER_FACTORY = LoggerFactory.getLogger(ResponseHelper.class);
    ...
    protected ResponseHelper(ServletContext context, HttpServletRequest request, HttpServletResponse response, UrlInfo urlInfo, String viewsPath) {
		...
		try {
			...
			LOGGER_FACTORY.error("Can't load framework configuration", e);
		}
        this.initParams();
    }
            ...
            if (urlInfo.getSerializer() == null) {
                ...
                if (this.object2Serialize == null) {
                    if (this.file != null) {
                        ...
                        LOGGER_FACTORY.warn("There is not object to serialize, returning no content response code: {}", HttpURLConnection.HTTP_NO_CONTENT);
    ...
    protected void serialize() throws IOException {
        try {
            LOGGER_FACTORY.debug("Serializing using {}", urlInfo.getSerializer());
            Class<?> clazz = Class.forName(urlInfo.getSerializer());
            LOGGER_FACTORY.debug("Creating a new instance of {}", urlInfo.getSerializer());
            ObjectSerializer serializer = (ObjectSerializer) clazz.newInstance();
            LOGGER_FACTORY.debug("Calling {}.serialize()", urlInfo.getSerializer());
            String serialized = serializer.serialize(this.object2Serialize);
            LOGGER_FACTORY.debug("Writing object in the response: {}", serialized);
            this.writeObject(serializer.getContentType(), serialized);
        } catch (Exception e) {
            LOGGER_FACTORY.error("Can't serialize object with {} serializer: {}", urlInfo.getSerializer(), e.getLocalizedMessage());
    ...
    protected void sendFile() throws IOException {
        try {
            LOGGER_FACTORY.debug("Sending file using {}", urlInfo.getSerializer());
            Class<?> clazz = Class.forName(urlInfo.getSerializer());
            LOGGER_FACTORY.debug("Creating a new instance of {}", urlInfo.getSerializer());
    ...
    protected void forward() throws IOException, ServletException {
        if (requestedPage == null || "".equals(requestedPage)) {
            ...
            try {
                ...
                LOGGER_FACTORY.debug("Forwarding to {}", requestedPage);
                dispatcher.forward(request, response);
            } catch (ServletException e) {
                LOGGER_FACTORY.error(e.getLocalizedMessage(), e);
                throw e;
            }
        }
    }
    ...
    private String searchPage(String pageWithoutExtension) {
        ...
        try {
            ...
            LOGGER_FACTORY.debug("URL for page is not well formed", e);
            page = "";
        }
        return page;
    }
    ...
    private Boolean existsPage(String page) throws MalformedURLException {
        // Searching the page...
        LOGGER_FACTORY.debug("Searching page [{}]...", page);
        LOGGER_FACTORY.debug("Page's real path is [{}]", this.context.getRealPath(page));
        File file = new File(this.context.getRealPath(page));
        Boolean exists = file.exists();
        LOGGER_FACTORY.debug("Page [{}]{}found", page, (exists ? " " : " not "));
        return exists;
    }
}
