 ...
 */
public class ClassScanerKit {
  protected static final Logger LOG = LoggerFactory.getLogger(ClassScanerKit.class);
  ...
  public static ClassScanerKit of(Class target) {
    return new ClassScanerKit(target);
  }
  ...
  private static List<String> findFiles(String baseDirName, String targetFileName) {
    ...
    try {
      baseURLs = ClassScanerKit.class.getClassLoader().getResources(File.separator + baseDirName.replaceAll("\\.", "/"));
    } catch (IOException e) {
      e.printStackTrace();
    }
    URL baseURL = null;
...
//      } else
    ...
    if (!baseDir.exists() || !baseDir.isDirectory()) {
      ...
      for (int i = 0; i < filelist.length; i++) {
        ...
        if (readfile.isDirectory()) {
          ...
          if (ClassScanerKit.wildcardMatch(targetFileName, tempName)) {
  ...
  public ClassScanerKit(Class target) {
    this.target = target;
  }

  public ClassScanerKit includepaths(String... classpaths) {
    if (classpaths != null) {
  ...
  public ClassScanerKit includepaths(List<String> classpaths) {
    if (classpaths != null) {
      for (String classpath : classpaths) {
        this.includepaths.add(classpath);
      }
  ...
  public <T> List<Class<? extends T>> search() {
    if (includepaths.size() <= 0) {
      ...
      try {
        resources = ClassScanerKit.class.getClassLoader().getResources("");
      } catch (IOException e) {
        e.printStackTrace();
      }
      URL resource = null;
